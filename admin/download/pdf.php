...
$sn = 0;
foreach(fetchitems('history','id') as $row){
    extract($row);
    $sn++;
    $pdf->Cell(15, 10, $sn, 1);
    foreach(fetchWhere('quiz','id',$quizid) as $quiz);
    $pdf->Cell(35, 10, $quiz['quizname'], 1);
    foreach(fetchWhere('user','userid',$userid) as $user);
