    ...
    <div class="row " >
        ...
        <main class="col-lg-9 col-12 p-3">
            <nav class="row p-2 border-bottom mt-2">
                <h3 class="col-7 mb-4">History</h3>
                <div class="col-5 row">                   
            ...
            <div class="col-12 mt-3">
                    <?php
                    if (mysqli_num_rows(fetchWhere('history','userid',$_SESSION['userid'])) > 0) {
                        ?>

                    <div class="input-group mb-3">
                        <button class="btn btn-secondary col-2" ><i class="ti ti-file"></i><a class="text-light" href="download/pdf" style=" text-decoration: none;">PDF</a></button>
                        <button class="btn btn-secondary col-2" ><i class="ti ti-folder "></i><a class="text-light" href="download/csv" style=" text-decoration: none;">CSV</a></button>
                    </div>
                    <?php }?>
                <div class=" p-3 bod" style="overflow-x:scroll;">
                 <table class="table table-striped " >
                        ...
                        <tbody>
                            <?php 
                            $pos = 1;
                                foreach(fetchWhere('history','userid',$_SESSION['userid']) as $key){
                                    extract($key);
                                  
                            ?>
                            <tr>
                                <td><?=$pos?></td>
                                <?php foreach(fetchWhere('quiz','id',$quizid) as $key2){extract($key2);}?>
                                <td><?=$quizname?></td>
                                <td><?=$score?></td>
                                <td><?=$correct?></td>
                                <td><?=$wrong?></td>
                                <td><?=$point?></td>
                                <td><a href="scoreboard?name=<?=$_SESSION['userid']?>&&quiz=<?=$id?>">View</a></td>
                            </tr>
